"""Мой корч API — FastAPI + PostgreSQL/SQLite, совместимый с Vercel Functions."""
...
# ошибка на старте, чтобы не тратить запросы на проде на битую эфемерную БД.
SQLITE_FALLBACK = bool(DATABASE_URL and DATABASE_URL.startswith("sqlite"))
if SQLITE_FALLBACK:
    import sqlite3
...
if SQLITE_FALLBACK:
    ...
    def database():
        ...
        # Обязательно включаем внешние ключи на каждом соединении для каскадных удалений
...
class RigConfigInput(BaseModel):
    coin: str = Field(default="", max_length=30)
...
app = FastAPI(title="Мой корч API", lifespan=lifespan)
...
def bootstrap(user: dict[str, Any] = Depends(authorized_user)) -> dict[str, Any]:
    with database() as connection:
        ...
        cards_by_rig: dict[str, list[dict[str, Any]]] = {}
        if rigs:
            ...
            for card in rows:
                cards_by_rig.setdefault(str(card["rig_id"]), []).append(card)
    return {"user": user_payload(user), "coins": coins, "rigs": rigs, "cardsByRig": cards_by_rig}


@app.get("/api/rigs")
def list_rigs(user: dict[str, Any] = Depends(authorized_user)) -> list[dict[str, Any]]:
    with database() as connection:
        return [dict(r) for r in run(connection, f"SELECT {RIG_FIELDS} FROM rigs WHERE user_id = %s ORDER BY id", (user["id"],)).fetchall()]
...
def create_rig(payload: RigInput, user: dict[str, Any] = Depends(authorized_user)) -> dict[str, Any]:
    with database() as connection:
        row = run(connection, f"INSERT INTO rigs(user_id, name) VALUES (%s, %s) RETURNING {RIG_FIELDS}", (user["id"], payload.name.strip())).fetchone()
        return dict(row)
...
def validate_coin_ownership(connection: Any, coin: str, user_id: int) -> None:
    if not coin:
        return
    if run(connection, "SELECT 1 FROM coins WHERE user_id = %s AND LOWER(name) = LOWER(%s)", (user_id, coin.strip())).fetchone() is None:
...
def delete_rig(rig_id: int, user: dict[str, Any] = Depends(authorized_user)) -> None:
    with database() as connection:
        result = run(connection, "DELETE FROM rigs WHERE id = %s AND user_id = %s", (rig_id, user["id"]))
...
def list_coins(user: dict[str, Any] = Depends(authorized_user)) -> list[dict[str, Any]]:
    with database() as connection:
        return [dict(r) for r in run(connection, "SELECT id, name, price_usd FROM coins WHERE user_id = %s ORDER BY name", (user["id"],)).fetchall()]
...
def create_coin(payload: CoinInput, user: dict[str, Any] = Depends(authorized_user)) -> dict[str, Any]:
    try:
        with database() as connection:
            row = run(connection, "INSERT INTO coins(user_id, name, price_usd) VALUES (%s, %s, %s) RETURNING id, name, price_usd", (user["id"], payload.name.strip().upper(), payload.price_usd)).fetchone()
            return dict(row)
    except Exception as error:
        if translate_integrity_error(error):
...
def update_coin(coin_id: int, payload: CoinInput, user: dict[str, Any] = Depends(authorized_user)) -> dict[str, Any]:
    ...
    try:
        with database() as connection:
            old = run(connection, "SELECT name FROM coins WHERE id = %s AND user_id = %s", (coin_id, user["id"])).fetchone()
            if old is None:
                raise HTTPException(status_code=404, detail="Монета не найдена")
            row = run(connection, "UPDATE coins SET name = %s, price_usd = %s WHERE id = %s AND user_id = %s RETURNING id, name, price_usd", (name, payload.price_usd, coin_id, user["id"])).fetchone()
            run(connection, "UPDATE rigs SET coin = %s WHERE user_id = %s AND LOWER(coin) = LOWER(%s)", (name, user["id"], old["name"]))
            return dict(row)
    except Exception as error:
        if translate_integrity_error(error):
            raise HTTPException(status_code=409, detail="Такая монета уже добавлена")
...
def delete_coin(coin_id: int, user: dict[str, Any] = Depends(authorized_user)) -> None:
    with database() as connection:
        coin = run(connection, "SELECT name FROM coins WHERE id = %s AND user_id = %s", (coin_id, user["id"])).fetchone()
        if coin is None:
            raise HTTPException(status_code=404, detail="Монета не найдена")
        if run(connection, "SELECT 1 FROM rigs WHERE user_id = %s AND LOWER(coin) = LOWER(%s) LIMIT 1", (user["id"], coin["name"])).fetchone():
            raise HTTPException(status_code=409, detail="Нельзя удалить монету, пока она используется в конфигурации рига")
        run(connection, "DELETE FROM coins WHERE id = %s AND user_id = %s", (coin_id, user["id"]))


def validate_rig_ownership(connection: Any, rig_id: int, user_id: int) -> None:
    if run(connection, "SELECT 1 FROM rigs WHERE id = %s AND user_id = %s", (rig_id, user_id)).fetchone() is None:
        raise HTTPException(status_code=404, detail="Риг не найден")
...
def list_cards(rig_id: int, user: dict[str, Any] = Depends(authorized_user)) -> list[dict[str, Any]]:
    with database() as connection:
        validate_rig_ownership(connection, rig_id, user["id"])
        return [dict(r) for r in run(connection, f"SELECT {CARD_FIELDS} FROM cards WHERE rig_id = %s AND user_id = %s ORDER BY id", (rig_id, user["id"])).fetchall()]
...
def create_card(payload: CardInput, user: dict[str, Any] = Depends(authorized_user)) -> dict[str, Any]:
    ...
    with database() as connection:
        ...
        row = run(connection, f"INSERT INTO cards(user_id, rig_id, model, quantity, hashrate, power) VALUES (%s, %s, %s, %s, %s, %s) RETURNING {CARD_FIELDS}", (user["id"], data["rig_id"], data["model"], data["quantity"], data["hashrate"], data["power"])).fetchone()
        return dict(row)
...
def update_card(card_id: int, payload: CardInput, user: dict[str, Any] = Depends(authorized_user)) -> dict[str, Any]:
    ...
    with database() as connection:
        validate_rig_ownership(connection, data["rig_id"], user["id"])
        row = run(connection, f"UPDATE cards SET rig_id=%s, model=%s, quantity=%s, hashrate=%s, power=%s WHERE id=%s AND user_id=%s RETURNING {CARD_FIELDS}", (data["rig_id"], data["model"], data["quantity"], data["hashrate"], data["power"], card_id, user["id"])).fetchone()
        if row is None:
...
def delete_card(card_id: int, user: dict[str, Any] = Depends(authorized_user)) -> None:
    with database() as connection:
        result = run(connection, "DELETE FROM cards WHERE id = %s AND user_id = %s", (card_id, user["id"]))
        if result.rowcount == 0:
            raise HTTPException(status_code=404, detail="Карта не найдена")
...
@app.api_route("/fonts/{file_name}", methods=["GET", "HEAD"])
def font_file(file_name: str) -> FileResponse:
    safe_name = Path(file_name).name
    if safe_name != file_name or not file_name.endswith((".ttf", ".woff2", ".otf")):
...
@app.api_route("/{file_name}", methods=["GET", "HEAD"])
