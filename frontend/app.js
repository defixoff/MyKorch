/**
 * Мой корч — NextGen PWA Mining Calculator (Mobile Polish Edition)
 * 
...
// Тактильный виброотклик на смартфонах (12ms)
...
// Расчёт дохода по конкретной видеокарте
...
function launchConfetti(originX = window.innerWidth / 2, originY = window.innerHeight / 2) {
  ...
  const count = Math.min(36, Math.floor(window.innerWidth / 12));
  const particles = [];
...
// Быстрое переключение темы оформления
function toggleTheme() {
  state.theme = state.theme === 'dark' ? 'light' : 'dark';
  applyTheme();
  haptic();
  renderDashboard();
  toast(state.theme === 'dark' ? 'Тёмная неоновая тема' : 'Светлая морозная тема');
}
...
// Плавная числовая интерполяция (Spring Tween)
...
// Интерактивная 3D-подсветка стекла
...
// Фоновый Canvas с кибер-частицами
function initBackgroundParticles() {
  ...
  const particleCount = isMobile ? 18 : Math.min(Math.floor(width / 35), 40);
  const particles = [];
  ...
  for (let i = 0; i < particleCount; i++) {
    particles.push({
      ...
      radius: Math.random() * (isMobile ? 1.4 : 1.8) + 1,
    });
  }
...
function renderAuth(mode = 'login') {
  app.innerHTML = `<section class="page-enter flex min-h-screen items-center justify-center p-4 sm:p-5">
    <div class="auth-panel glass w-full max-w-md rounded-3xl p-6 sm:p-9">
      <div class="mb-7 sm:mb-8">
        <div class="auth-logo mb-4">${ico('logo', 'ico-lg')}</div>
        <h1 class="auth-title">Мой корч</h1>
        <p class="mt-2 text-sm sm:text-[15px] text-slate-500 dark:text-slate-400">Премиальный майнинг-калькулятор фермы. Точный расчёт по каждому ригу и каждой видеокарте.</p>
      </div>
      ...
      <button id="auth-switch" class="mt-5 w-full text-sm font-semibold text-amber-500 transition hover:text-amber-400 py-1">
        ${mode === 'login' ? 'Нет аккаунта? Зарегистрироваться' : 'Уже есть аккаунт? Войти'}
      </button>
      ${mode === 'login' ? `<button id="auth-forgot" class="mt-2 w-full text-xs font-medium text-slate-500 transition hover:text-amber-500 py-1">Забыли пароль?</button>` : ''}
    </div>
  </section>`;
...
function renderDashboard() {
  ...
  // Динамические показатели сверху: по умолчанию для активного рига!
  ...
  app.innerHTML = `<div class="page-enter mx-auto min-h-screen max-w-6xl p-3.5 sm:p-7">
    <!-- Шапка с быстрыми действиями (Touch-Friendly) -->
    <header class="mb-5 sm:mb-6 flex items-center justify-between gap-2.5">
      <div class="flex items-center gap-2.5 sm:gap-3">
        <div class="auth-logo" style="width:2.65rem;height:2.65rem;border-radius:14px">${ico('logo', 'ico-lg')}</div>
        <div>
          <div class="flex items-center gap-1.5">
            <span class="status-pulse" title="Ферма онлайн"></span>
            <p class="text-[10px] sm:text-[11px] font-bold uppercase tracking-[.2em] text-amber-500/95 truncate max-w-[140px] sm:max-w-none">${escapeHtml(state.user.username)}</p>
          </div>
          <h1 class="font-display text-[22px] font-extrabold sm:text-3xl leading-tight">Мой корч</h1>
        </div>
      </div>
      <div class="flex items-center gap-1.5 sm:gap-2.5">
        <button id="btn-theme-quick" class="btn btn-ghost btn-icon" title="Сменить тему оформления" aria-label="Сменить тему">
          ${ico(state.theme === 'dark' ? 'sun' : 'moon')}
        </button>
        <button id="settings" class="btn btn-ghost" title="Настройки фермы">
          ${ico('gear')} <span class="hidden sm:inline">Настройки</span>
        </button>
        <button id="logout-button" class="btn btn-ghost btn-icon" title="Выйти" aria-label="Выйти">${ico('logout')}</button>
      </div>
    </header>

    <!-- Панель управления (Сегментированные контролы, оптимизированные под мобильные) -->
    <section class="mb-4 sm:mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
      <div class="w-full sm:w-auto">
        <div class="horizon-bar w-full sm:w-auto grid grid-cols-2 sm:flex">
      ...
      <div class="w-full sm:w-auto">
        <div id="horizon-selector" class="horizon-bar w-full sm:w-auto grid grid-cols-4 sm:flex">
    ...
    <!-- Компактная сводка всей фермы -->
    <div class="farm-summary-bar glass mb-4 sm:mb-5 rise-in">
      <div class="flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
        <span class="chip chip-amber">${ico('farm')} Ферма: ${state.rigs.length} риг. (${farmTotals.cardsCount} карт)</span>
        ...
        <span>· Профит: <b class="num ${farmTotals.profit >= 0 ? 'text-emerald-500' : 'text-rose-400'}">${money(farmTotals.profit * mult)}</b></span>
      </div>
      <span class="text-[11px] font-semibold text-slate-400 dark:text-slate-500 hidden sm:inline">Период: ${horizon.label}</span>
    </div>

    <!-- ГЛАВНЫЕ КАРТОЧКИ СВЕРХУ (динамически привязаны к активному ригу) -->
    <section class="grid gap-3 sm:gap-4 sm:grid-cols-2">
      ...
      <article class="stat-card card-glass rise-in stat-main ${isProfitNegative ? 'negative' : 'profitable'}" data-stat-scope="profit">
        <div class="stat-label">
          <span class="truncate flex items-center gap-1.5">
            ${ico('coin')} Чистый профит · <b class="text-slate-900 dark:text-slate-100 truncate">${escapeHtml(isRigScope ? (selectedRig?.name || 'Риг не выбран') : 'Вся ферма')}</b>
          </span>
          <span class="chip shrink-0 ${isProfitNegative ? 'chip-rose' : 'chip-emerald'}">
        ...
        <div class="mt-2.5 sm:mt-3 flex flex-wrap items-center justify-between gap-1.5 text-xs text-slate-500 dark:text-slate-400">
      ...
      <article class="stat-card card-glass rise-in stat-card-power" style="animation-delay:60ms" data-stat-scope="power">
        <div class="stat-label">
          <span class="truncate flex items-center gap-1.5">
            ${ico('flash')} Расход на свет · <b class="text-slate-900 dark:text-slate-100 truncate">${escapeHtml(isRigScope ? (selectedRig?.name || 'Риг не выбран') : 'Вся ферма')}</b>
          </span>
        ...
        <div class="mt-2.5 sm:mt-3 flex flex-wrap items-center justify-between gap-1.5 text-xs text-slate-500 dark:text-slate-400">
          <span class="truncate">${isRigScope && selectedRig ? `Тариф: ${money(selectedRig.electricity_cost)}/кВт·ч` : 'Суммарный свет фермы'}</span>
          <span class="shrink-0">за ${horizon.label}</span>
        </div>
    ...
    <!-- Секция выбора ригов (Табы со скроллом и пилюлей) -->
    <section class="mt-6 sm:mt-8">
      <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2">
          <h2 class="font-display text-base sm:text-lg font-bold">Риги фермы</h2>
          <span class="text-xs font-semibold text-slate-500">(${state.rigs.length})</span>
        </div>
        <button id="add-rig" class="btn btn-ghost btn-sm">${ico('plus')} Добавить риг</button>
      </div>
    ...
    <section class="card-glass rise-in mt-5 sm:mt-6 p-4 sm:p-7" style="animation-delay:140ms">
      <div class="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          ${selectedRig ? `
            <div class="mt-1.5 flex flex-wrap items-center gap-2 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
        ...
        ${selectedRig ? `
          <div class="flex shrink-0 gap-1.5">
            <button id="rig-config" class="btn btn-ghost" title="Конфигурация рига">${ico('gear')} <span class="hidden sm:inline">Конфигурация</span></button>
      ...
      <!-- Сетка видеокарт с РАСЧЁТОМ ДОХОДА НА КАЖДУЮ КАРТУ -->
      <div class="mb-3.5 flex items-center justify-between">
        <h3 class="font-display text-sm sm:text-base font-bold flex items-center gap-2">
          ${ico('gpu')} Видеокарты рига
          <span class="chip text-xs">${state.cards.reduce((sum, c) => sum + Number(c.quantity || 1), 0)} шт.</span>
        </h3>
        ${selectedRig ? `<button id="add-card" class="btn btn-accent">${ico('plus')} Добавить карту</button>` : ''}
      </div>

      <div id="card-list" class="grid gap-3 sm:gap-3.5 sm:grid-cols-2">
        ${state.cards.length ? state.cards.map((card, index) => cardHtml(card, selectedRig, rigTotals, mult, horizon.suffix, index)).join('') : `
          <div class="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-slate-500 dark:border-slate-800 sm:col-span-2">
            <div class="mx-auto mb-2.5 w-10 text-slate-400">${ico('gpu', 'ico-lg')}</div>
            <p class="font-bold text-sm sm:text-base">В этом риге ещё нет видеокарт</p>
            <p class="mt-1 text-xs text-slate-400">Нажмите кнопку «Добавить карту», чтобы рассчитать доход и хешрейт.</p>
          </div>
        `}
      </div>
      ...
      ${selectedRig ? `
        <footer class="rig-footer mt-6 flex flex-col gap-4 border-t border-white/5 pt-5 sm:flex-row sm:items-center sm:justify-between">
          <div data-stat-scope="rig-summary">
            ...
            <p class="mt-1 text-xl font-extrabold sm:text-2xl">
              ...
              <span class="text-xs sm:text-sm font-medium text-slate-500">чистыми / свет <span class="num text-slate-400" data-tween="rig-footer-electricity" data-tween-fmt="money">${money(rigTotals.electricityDaily * mult)}</span></span>
            </p>
            <p class="rig-total-hash font-display num mt-1.5 text-[15px] sm:text-[16px] font-extrabold text-amber-500/95">
          ...
          <div class="flex flex-wrap gap-2">
            <button id="btn-add-card-footer" class="btn btn-accent w-full sm:w-auto">${ico('plus')} Добавить карту</button>
          </div>
        </footer>
      ` : ''}
    </section>
  ...
  // Обработчики шапки
  document.querySelector('#btn-theme-quick')?.addEventListener('click', toggleTheme);
  ...
  // Селекторы фокуса
  document.querySelector('#scope-rig')?.addEventListener('click', () => { haptic(); switchViewScope('rig'); });
  document.querySelector('#scope-farm')?.addEventListener('click', () => { haptic(); switchViewScope('farm'); });
...
// Генерация карточки видеокарты с расчётом дохода
  ...
  return `<article style="animation-delay:${Math.min(index, 10) * 45}ms" class="gpu-card card-pop" data-card-item="${card.id}">
    <div class="flex items-start justify-between gap-2.5">
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-1.5">
          <span class="status-pulse shrink-0" title="В работе"></span>
          <h4 class="font-display font-bold text-sm sm:text-base tracking-tight truncate">${escapeHtml(card.model)}</h4>
        </div>
        <div class="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] sm:text-xs text-slate-500 dark:text-slate-400">
          <span class="chip chip-amber">${calc.qty} шт.</span>
          <span>${number(calc.unitHash)} MH/s · ${number(calc.unitPower, 0)} Вт/шт</span>
        </div>
      </div>
    ...
    <!-- Примерный чистый доход видеокарты -->
    <div class="gpu-profit-badge ${isProfitable ? 'profitable' : 'negative'}">
      <div class="flex items-baseline justify-between gap-1.5">
        <span class="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-500">Примерный чистый профит</span>
        <span class="chip ${isProfitable ? 'chip-emerald' : 'chip-rose'} text-[10px] sm:text-[11px]">
          ${calc.margin > 0 ? `+${calc.margin.toFixed(0)}%` : `${calc.margin.toFixed(0)}%`} маржа
        </span>
      </div>
      ...
      ${calc.qty > 1 ? `
        <span class="mt-0.5 text-[10px] sm:text-[11px] font-medium text-slate-400">
    ...
    <div class="gpu-stat-grid text-xs">
      <div class="gpu-stat-chip">
        <span class="block text-[10px] sm:text-[11px] font-semibold text-slate-500">Грязный доход</span>
        <b class="num text-[12px] sm:text-[13px] text-slate-800 dark:text-slate-200">${money(gross)}</b>
        <span class="block text-[9px] sm:text-[10px] text-slate-400 truncate">~${number(coins, 4)} ${escapeHtml(rig?.coin || '')}</span>
      </div>

      <div class="gpu-stat-chip">
        <span class="block text-[10px] sm:text-[11px] font-semibold text-slate-500">Расход на свет</span>
        <b class="num text-[12px] sm:text-[13px] text-slate-800 dark:text-slate-200">${money(electricity)}</b>
        <span class="block text-[9px] sm:text-[10px] text-slate-400 truncate">${number(calc.totalPower, 0)} Вт суммарно</span>
      </div>

      <div class="gpu-stat-chip">
        <span class="block text-[10px] sm:text-[11px] font-semibold text-slate-500">Общий хеш</span>
        <b class="num text-[12px] sm:text-[13px] text-amber-500">${number(calc.totalHash)} MH/s</b>
        <span class="block text-[9px] sm:text-[10px] text-slate-400 truncate">${calc.rigShare.toFixed(1)}% от рига</span>
      </div>

      <div class="gpu-stat-chip">
        <span class="block text-[10px] sm:text-[11px] font-semibold text-slate-500">Эффективность</span>
        <b class="num text-[12px] sm:text-[13px] text-slate-800 dark:text-slate-200">${calc.wattPerMhs.toFixed(2)}</b>
        <span class="block text-[9px] sm:text-[10px] text-slate-400 truncate">Вт / (MH/s)</span>
      </div>
    </div>
...
function switchRig(rigId) {
  ...
  document.querySelectorAll('.rig-tab').forEach((tab) => {
    const isActive = Number(tab.dataset.rigId) === Number(rigId);
...
// Создание модального окна (с поддержкой Mobile Bottom Sheet)
function modal(title, content) {
  const node = document.createElement('div');
  node.className = 'modal-backdrop';
  node.innerHTML = `
    <div class="modal-panel">
      <div class="modal-handle sm:hidden" aria-hidden="true"></div>
      <div class="mb-4 flex items-center justify-between gap-3">
        <h2 class="modal-title font-display text-base sm:text-lg font-bold"></h2>
...
// Модалка конфигурации рига (с inputmode для мобильных)
function showRigConfigModal(rig) {
  ...
  const node = modal(`Конфигурация: ${rig.name}`, `
    <form data-rig-config="${rig.id}" class="space-y-3.5">
      ...
      <div class="grid grid-cols-2 gap-3">
        <label class="block text-sm font-semibold">Добыча/сутки (риг)
          <input name="coin_per_day" type="number" inputmode="decimal" min="0" step="any" required value="${rig.coin_per_day ?? 0}" class="field">
        </label>
        <label class="block text-sm font-semibold">Розетка, $/кВт·ч
          <input name="electricity_cost" type="number" inputmode="decimal" min="0" step="0.001" required value="${rig.electricity_cost ?? 0.1}" class="field">
        </label>
      </div>
      <div class="glass rounded-2xl p-3.5">
        <p class="mb-2 flex items-center gap-2 text-sm font-bold">${ico('clock')} График смен</p>
        <div class="grid grid-cols-2 gap-2.5">
          <label class="text-xs font-bold text-slate-500">Рабочих суток (18ч)
            <input name="working_days" type="number" inputmode="numeric" min="0" max="31" step="1" required value="${rig.working_days ?? 5}" class="field">
          </label>
          <label class="text-xs font-bold text-slate-500">Выходных суток (24ч)
            <input name="weekend_days" type="number" inputmode="numeric" min="0" max="31" step="1" required value="${rig.weekend_days ?? 2}" class="field">
          </label>
        </div>
...
// Модалка добавления / редактирования видеокарты (с цифровой клавиатурой)
function showCardModal(card = null) {
  ...
  modal(isEdit ? 'Редактировать карту' : 'Добавить карту', `
    <form id="card-form" data-card-id="${card?.id || ''}" class="space-y-3.5">
      <label class="block text-sm font-semibold">Модель видеокарты
      ...
      <div class="grid grid-cols-2 gap-3">
        <label class="block text-sm font-semibold">Количество (шт.)
          <input name="quantity" type="number" inputmode="numeric" required min="1" step="1" value="${values.quantity}" class="field">
        </label>
        <label class="block text-sm font-semibold">Хешрейт (1 шт, MH/s)
          <input name="hashrate" type="number" inputmode="decimal" required min="0" step="any" value="${values.hashrate}" class="field" placeholder="65.5">
        </label>
      </div>
      <label class="block text-sm font-semibold">Мощность (1 шт, Вт)
        <input name="power" type="number" inputmode="numeric" required min="0" step="1" value="${values.power}" class="field" placeholder="180">
      </label>
      <button class="btn btn-accent w-full">${isEdit ? 'Сохранить изменения' : 'Добавить карту'}</button>
    </form>
  `);
}
...
function showSettingsModal() {
  ...
  const node = modal('Настройки фермы', `
    <div class="glass mb-4 flex items-center justify-between gap-3 rounded-2xl p-3">
      <div>
        <p class="text-sm font-bold">Тема интерфейса</p>
        <p class="text-xs text-slate-500">Тёмная неоновая аврора / светлый морозный кристалл</p>
      </div>
    ...
    <div id="settings-coin-list" class="space-y-2 max-h-56 overflow-y-auto pr-0.5"></div>

    <form id="settings-new-coin" class="mt-3 flex flex-wrap sm:grid sm:grid-cols-[1fr_1fr_auto] items-end gap-2 border-t border-white/5 pt-3">
      <label class="flex-1 min-w-[90px] text-xs font-bold text-slate-500">Монета
        <input name="name" required maxlength="30" placeholder="BTC / ETH" class="settings-field">
      </label>
      <label class="flex-1 min-w-[90px] text-xs font-bold text-slate-500">Курс, $
        <input name="price_usd" type="number" inputmode="decimal" required min="0" step="any" placeholder="1.00" class="settings-field">
      </label>
      <button class="btn btn-accent btn-icon shrink-0 self-end" title="Добавить монету" aria-label="Добавить монету">${ico('plus')}</button>
    </form>

    <details class="glass mt-3.5 rounded-2xl p-3">
  ...
  node.querySelector('#settings-theme-toggle').onclick = () => {
    toggleTheme();
    node.querySelector('#settings-theme-toggle').innerHTML = ico(state.theme === 'dark' ? 'sun' : 'moon');
  };
  ...
  const renderCoins = () => {
    coinPanel.innerHTML = state.coins.map((coin) => `
      <form data-settings-coin="${coin.id}" class="glass flex flex-wrap sm:grid sm:grid-cols-[1fr_1fr_auto] items-end gap-2 rounded-2xl p-2.5">
        <label class="flex-1 min-w-[90px] text-xs font-bold text-slate-500">Монета
          <input name="name" required maxlength="30" value="${escapeHtml(coin.name)}" class="settings-field">
        </label>
        <label class="flex-1 min-w-[90px] text-xs font-bold text-slate-500">Курс, $
          <input name="price_usd" type="number" inputmode="decimal" required min="0" step="any" value="${coin.price_usd}" class="settings-field">
        </label>
        <div class="flex gap-1.5 shrink-0 self-end">
          <button title="Сохранить курс" class="btn btn-accent btn-icon" aria-label="Сохранить">${ico('check')}</button>
          <button type="button" data-settings-delete="${coin.id}" title="Удалить" class="btn btn-ghost btn-icon danger" aria-label="Удалить">${ico('trash')}</button>
        </div>
      </form>
...
async function init() {
  ...
  try {
    ...
    if (!state.user) {
      app.innerHTML = `
        <section class="page-enter flex min-h-screen items-center justify-center p-4 sm:p-6 text-center">
          <div class="glass max-w-sm rounded-3xl p-6 sm:p-7">
